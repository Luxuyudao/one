 ...
 * 验证码生成类
 */
public class XImageCaptchaServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		key = key.trim();
		if (key.length() != 32 && !key.startsWith(Constant.IMG_VERIFY_CODE_KEY)) {
			return;
		}
		...
		// session.setAttribute(BasUserConstantConfig.SESSION_PIC_CODE_KEY,
		// validationCode);
		// ManagerConfig.jedisClient.set(key, validationCode, "nx", "ex",
		// Constant.IMG_VERIFY_EFFECTIVE_TIME);

		JedisClient jedisClient = RedisCacheHelper.getJedisClient("redisCache");
		CacheUtil.redisPut(jedisClient, key, validationCode, Constant.IMG_VERIFY_EFFECTIVE_TIME);
