...
<body>
		...
		<div id="main-content" class="clearfix" data-page="projectcenter" sub-page="addproject">
			<div id="breadcrumbs">
				<ul class="breadcrumb">
					<li><i class="icon-home"></i><a href="http://advisor.demo1.com/index.jsp">首页</a><span class="divider"><i class="icon-angle-right"></i> </span>
					</li>
					<li><a href="http://advisor.demo1.com/project_list.jsp">项目列表</a><span class="divider"><i class="icon-angle-right"></i> </span></li>
					<li>任务终止</li>
				</ul>
			</div>
			<div id="page-content" class="clearfix">
				...
				<div class="row-fluid">
					...
					<div class="row-fluid mar-t-15">
						<div class="span12">
							<form class="form-inline bord1dd form-add1">
								...
								<div class="form-group mar-t-15">
									<label>服务商</label> <span id="adviser_name"></span>
								</div>
								<div class="form-group mar-t-15">
									<label>任务周期</label> <span id="task_cycle"></span> <span
										id="cycle_unit_name"></span>
								</div>
								<div class="form-group mar-t-15">
									<label>任务酬劳</label> <span id="real_amount"></span>元
								</div>
								<div class="form-group mar-t-15">
								...
								<div class="form-group mar-t-15">
									<label class="div-inline">任务内容</label>
		            ...
		            <div class="box border mt20">
		             ...
		             <div class="box border mt20">
		              	<div class="box-title">
		                	<h4><i class="fa fa-table"></i>终止原因</h4>
		              	</div>
		              	<div class="form-inline bord1dd form-add1 font14">
		              		<div class="form-group mar-t-15">
		              		<label></label>
								<textarea id="yn_zz" class="form-control mar-t-15" style="width:50%;" rows="4"></textarea>
							</div>
		             	</div>
		            </div>
					<div class="tc mar-t-20">
						<button id="tj_btn" type="button" class="btn btn-submit"
							data-toggle="button">任务终止</button>
	
						<button id="fh_btn" type="button" class="btn btn-submit" data-toggle="button">返回</button>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#tj_btn").click(function() {
			var xco = new XCO();
			xco.setStringValue("termination_desc", $("#yn_zz").val());
			xco.setStringValue("project_task_sn",getURLparam("project_task_sn"));
			var options = {
				url : "/project-operate/updateTaskP45.xco",
				data : xco,
				success : getupdateTaskP45CallBack
			};
			jConfirm("确认终止该任务吗？","确认框",function(r){
				if(r){
	...
	function getupdateTaskP45CallBack(data) {
		if (data.getCode() != 0) {
			tvrAlert('doCallBack error:\n' + data.getMessage());
		} else {
			jAlert("任务已终止","提示",function(r){
				window.location.href="project_implement.jsp?project_sn="+project_sn;
			});
		}
	}
	getMyProjectTaskLists(0, globle_page_size);

	function getMyProjectTaskLists(_start, _pageSize) {
		var xco = new XCO();
		xco.setStringValue("project_task_sn", getURLparam("project_task_sn"));
		...
		var options = {
			...
			data : xco,
			success : getMyProjectTaskListCallBacks
		};
		$.doXcoRequest(options);
	}
	
	var renderPage = true;
	function getMyProjectTaskListCallBacks(data) {
		if (data.getCode() != 0) {
			tvrAlert('doCallBack error:\n' + data);
		} else {
			...
			$("#address").text(task.getStringValue("task_province")
							+ task.getStringValue("task_city")
							+ task.getStringValue("task_area")
							+ task.getStringValue("task_addr"));
			$("#adviser_name").text(task.getStringValue("adviser_name"));
			$("#task_type").text(task.getStringValue("task_type"));
			$("#task_desc").text(task.getStringValue("task_desc"));
			$("#task_cycle").text(task.getStringValue("task_cycle"));
			$("#cycle_unit_name").text(task.getStringValue("cycle_unit_name"));
			$("#real_amount").text(parseInt(task.getStringValue('real_amount')) / 100);
			...
			if (renderPage) {
				renderPage = false;
				pageUtil('pagination_1', parseInt(total), getMyProjectTaskLists,
						globle_page_size);
			};
	...
	getMyProjectTaskLists(project_task_sn);
</script>
