		...
		<div id="main-content" class="clearfix" data-page="projectcenter" sub-page="addproject">
			<div id="breadcrumbs">
				<ul class="breadcrumb">
					...
					<li><a href="http://provider.demo1.com/project_list.jsp">任务列表</a><span class="divider"><i
			...
			<div id="page-content" class="clearfix">
				...
				<div class="row-fluid">
					...
					<div class="box border mt20">
		              <div class="box-title">
		                <h4><i class="fa fa-table"></i> 任务进度 <a id="add_btn" href="javascript:void(0);" style="font-size: 12px;" onclick="add()">新增日报</a></h4>
...
<script type="text/javascript">
	function back(){
		window.location.href="http://provider.demo1.com/project_list.jsp";
	}
	$("#add_btn").hide();
	var project_sn='';
...
/* 			if((task.getIntegerValue('task_notpass_mark')==1) || (task.getIntegerValue("task_ctrl_state")==1&&task.getIntegerValue("task_state")==30&&task.getIntegerValue("task_progress")<100)){
				$("#add_btn").show();
			}else if((task.getIntegerValue('task_notpass_mark')==0) && (task.getIntegerValue("task_progress")<100)){
				$("#add_btn").show();
			}else{
				$("#add_btn").hide();
			}
 */			if(task.getIntegerValue('task_state')==30){
				$("#add_btn").show();
			}else{
				$("#add_btn").hide();
			}
	...
	function add(){
		layer.open({
			...
			shadeClose: true, //点击遮罩关闭层
			area : ["800px" , "550px"],
			content: ['add_daily.jsp?project_sn='+project_sn+'&project_task_sn='+getURLparam("project_task_sn")]
		});
	}
